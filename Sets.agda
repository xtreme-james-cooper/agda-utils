...
filterDoesNotAdd : {A : Set} {n n' : nat} {p : A -> Set} (a : A) (as : vect A n) (as' : vect A n') (f : (a : A) -> decide (p a)) -> filter f as == (n' , as') ->
  a ∈ as' -> a ∈ as
filterDoesNotAdd a []         .[]          f Refl (() , i)
filterDoesNotAdd a (b :: as)  as'          f eq   (ix , i)    with f b | filter f as | inspect (filter f) as
filterDoesNotAdd a (.a :: as) .(a :: as'') f Refl (FZ , Refl) | Yes _  | n' , as''   | [ pf ] = FZ , Refl
filterDoesNotAdd a (b :: as)  .(b :: as'') f Refl (FS ix , i) | Yes _  | n' , as''   | [ pf ] with filterDoesNotAdd a as as'' f pf (ix , i)
filterDoesNotAdd a (b :: as)  .(b :: as'') f Refl (FS ix , i) | Yes _  | n' , as''   | [ pf ] | (ix' , i') = FS ix' , i'
filterDoesNotAdd a (b :: as)  .as''        f Refl (ix , i)    | No nfa | n' , as''   | [ pf ] with filterDoesNotAdd a as as'' f pf (ix , i)
filterDoesNotAdd a (b :: as)  .as''        f Refl (ix , i)    | No nfa | n' , as''   | [ pf ] | (ix' , i') = FS ix' , i'
...
differenceDoesNotAdd : {A : Set} {n m n' : nat} {a : A} (as : vect A n) (bs : vect A m) (as' : vect A n') (aeq : equality A) -> difference aeq as bs == (n' , as') ->
  a ∈ as' -> a ∈ as
differenceDoesNotAdd {A} {n} {m} {n'} {a} as bs as' aeq eq mem = filterDoesNotAdd a as as' (λ a -> decideNot (contains? aeq bs a)) eq mem
...
removeDoesNotAdd : {A : Set} {n n' : nat} (as : vect A n) (a b : A) (as' : vect A n') (aeq : equality A) -> remove aeq as a == (n' , as') -> b ∈ as' -> b ∈ as
removeDoesNotAdd {A} {n} {n'} as a b as' aeq eq mem = filterDoesNotAdd b as as' (λ a' -> decideNot (aeq a a')) eq mem 
